...
def apply_hidden_pairs(values):
    # Itérer sur toutes les unités du Sudoku (lignes, colonnes, blocs)
    for unit in unitlist:
        # Compter les occurrences de chaque chiffre dans le unit
        ...
        # Chercher les paires cachées
        for digit, squares in occurence_map.items():
            if len(squares) == 2:  # Si un chiffre apparaît exactement dans 2 cases
                # Trouver un autre chiffre qui forme une paire cachée avec le chiffre actuel
...
def search(values):
    ...
    values = apply_hidden_pairs(values) # Appliquer hidden pairs
...
def solve_all(grids, name='', showif=0.0):
    ...
    N = len(grids)
    if N > 1:
        print ("Solved %d of %d %s puzzles (avg %.6f secs (%d Hz), max %.6f secs)." % (
